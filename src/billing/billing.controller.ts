...
export class BillingController {
  ...
  async getWallet(@Request() req) {
    const adminId = req.user.role === UserRole.SUPER_ADMIN ? req.query.adminId || req.user.id : req.user.id;
  ...
  @Get('transactions')
  @Roles(UserRole.ADMIN, UserRole.SUPER_ADMIN)
  async getTransactions(@Request() req, @Query() filter: TransactionFilterDto) {
    const adminId = req.user.role === UserRole.SUPER_ADMIN && req.query.adminId ? req.query.adminId : req.user.id;
    return this.billingService.getTransactions(adminId, filter);
  }

  @Get('transactions/all')
  @Roles(UserRole.SUPER_ADMIN)
  async getAllTransactions(@Query() filter: TransactionFilterDto) {
  ...
  async requestWithdrawal(@Body() dto: CreateWithdrawalDto, @Request() req) {
  ...
  @Get('withdrawals')
  @Roles(UserRole.ADMIN, UserRole.SUPER_ADMIN)
  async getWithdrawals(@Request() req, @Query('status') status?: string, @Query('page') page?: number, @Query('limit') limit?: number) {
    const adminId = req.user.role === UserRole.SUPER_ADMIN && req.query.adminId ? req.query.adminId : req.user.id;
    const withdrawalStatus = status as any;
    return this.billingService.getWithdrawalRequests(adminId, withdrawalStatus, page, limit);
  }

  @Get('withdrawals/all')
  @Roles(UserRole.SUPER_ADMIN)
  async getAllWithdrawals(@Query('status') status?: string, @Query('page') page?: number, @Query('limit') limit?: number) {
    const withdrawalStatus = status as any;
    return this.billingService.getWithdrawalRequests(undefined, withdrawalStatus, page, limit);
  }
  ...
  @Roles(UserRole.SUPER_ADMIN)
  async rejectWithdrawal(@Param('id') id: string, @Body() dto: UpdateWithdrawalStatusDto, @Request() req) {
    return this.billingService.rejectWithdrawal(id, dto.rejectionReason, req.user.id);
  }
  ...
  @Roles(UserRole.ADMIN)
  async recordClientPayment(@Body() dto: CreateClientPaymentDto, @Request() req) {
    if (dto.adminId !== req.user.id) {
  ...
  @Get('client-payments')
  @Roles(UserRole.ADMIN, UserRole.SUPER_ADMIN)
  async getClientPayments(@Request() req, @Query('page') page?: number, @Query('limit') limit?: number) {
    return this.billingService.getClientPayments(req.user.id, page, limit);
  }

  @Patch('client-payments/:id/mark-paid')
  @Roles(UserRole.ADMIN)
  async markPaymentAsPaid(@Param('id') id: string) {
    return this.billingService.markPaymentAsPaid(id);
  }
