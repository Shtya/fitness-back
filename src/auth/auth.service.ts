...
export class AuthService {
	...
	async adminGenerateTempCredentials(targetUserId: string, actor: { id: string; role: UserRole }) {
		const user = await this.userRepo.findOne({ where: { id: targetUserId } });
		if (!user) throw new NotFoundException('User not found');

		// Admin scoping: admin can only reset users under them (optional but recommended)
		if (actor.role === UserRole.ADMIN) {
			if ((user.adminId ?? null) !== actor.id) throw new ForbiddenException('Not allowed');
		}

		// generate temp password
		const tempPass = crypto.randomBytes(6).toString('base64url'); // ~8 chars, adjust if you want longer
		user.password = await bcrypt.hash(tempPass, 12);

		// optional: mark user active, or keep status same
		// user.status = UserStatus.ACTIVE;

		await this.userRepo.save(user);

		// optional: email it too (if you want)
		try {
			await this.emailService.sendWelcomeWithPassword(user.email, user.name, tempPass);
		} catch { }

		const loginUrl = `${process.env.NEXT_PUBLIC_WEBSITE_URL || ''}/en/auth`;
	...
	async updateUserProfile(userId: string, dto: any) {
		...
		const allowedFields = ['name', 'phone', 'gender', 'membership', 'defaultRestSeconds', 'FiberTarget', 'caloriesTarget', 'proteinPerDay', 'carbsPerDay', 'fatsPerDay', 'activityLevel', 'notes'];
	...
	async updateProfile(userId: string, dto: UpdateProfileDto) {
		...
		set('FiberTarget' as any, dto.FiberTarget ?? null);
